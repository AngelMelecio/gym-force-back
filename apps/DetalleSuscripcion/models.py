...
class DetalleSuscripcion(models.Model):
    ...
    def __str__(self):
        return "DetalleSuscripcion: %s %s %s %s %s %s %s" % (self.idSuscripcion, self.idVenta, self.fechaInicio, self.fechaFin, float(self.precioVenta), float(self.descuento), self.estado)
